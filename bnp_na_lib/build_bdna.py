...
# Public constant retained for older imports and GUI/default display compatibility.
TEMPLATES = {
...
def build_bdna(
    ...
    base = sanitize_basename(base_name or f"B-DNA{len(seq_expanded)}")
    if not base:
    ...
    try:
        ...
        logs += [
            f"Wrote B-DNA helical-parameter table for {len(seq_expanded)} bp:",
            str(table_path),
...
# Backward-compatible alias for older imports while bnp_na.py uses build_bdna.
build_bdna_align2z = build_bdna
