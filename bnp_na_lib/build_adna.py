...
def build_adna(
    ...
    base = sanitize_basename(base_name or f"A-DNA{len(seq_expanded)}")
    if not base:
    ...
    try:
        ...
        logs += [
            f"Wrote A-DNA helical-parameter table for {len(seq_expanded)} bp:",
            str(table_path),
...
# Backward-compatible alias for older imports while bnp_na.py uses build_adna.
build_adna_align2z = build_adna
