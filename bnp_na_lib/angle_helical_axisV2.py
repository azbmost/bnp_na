...
#   python angle_helical_axisV2.py -i helix.pdb --point1 A:5:C1' --point2 B:18:C1' \
#       -o helix_axis_vectors.bild
#
# Example (custom axis, no PDB needed):
#   python angle_helical_axisV2.py --axis-point "0 0 0" --axis-vector "0 0 1" \
...
def write_bild(
    ...
    *,
    ...
    a1, a2 = axis.endpoints(margin=5.0)

    lines: List[str] = []
    lines.append(".comment Generated by angle_helical_axisV2.py")
...
def launch_gui(parent=None) -> None:
    ...
    root.title("angle_helical_axisV2.py")
    ...
    # Radii
    tk.Label(frm, text="Axis radius", anchor="w").grid(row=9, column=0, sticky="w")
    tk.Entry(frm, textvariable=var_axis_r, width=10).grid(row=9, column=1, sticky="w", padx=(5, 5))

    tk.Label(frm, text="Vector radius", anchor="w").grid(row=10, column=0, sticky="w")
    tk.Entry(frm, textvariable=var_vec_r, width=10).grid(row=10, column=1, sticky="w", padx=(5, 5))

    tk.Label(frm, text="Sphere radius", anchor="w").grid(row=11, column=0, sticky="w")
    tk.Entry(frm, textvariable=var_sph_r, width=10).grid(row=11, column=1, sticky="w", padx=(5, 5))

    tk.Button(frm, text="Run", command=do_run).grid(row=12, column=0, pady=(8, 8), sticky="w")

    # Output text
    text = ScrolledText(frm, height=18, width=90)
    text.grid(row=13, column=0, columnspan=3, sticky="nsew", pady=(5, 0))
